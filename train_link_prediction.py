...
parser = argparse.ArgumentParser('GLEN self-supervised training')
parser.add_argument('-d', '--data', type=str, help='Dataset name (eg. wikipedia or reddit)',
                    default='wikipedia')
parser.add_argument('--bs', type=int, default=200, help='Batch_size')
parser.add_argument('--prefix', type=str, default='glen', help='Prefix to name the checkpoints')
...
for i in range(args.n_runs):
  ...
  optimizer = torch.optim.Adam(glen.parameters(), lr=LEARNING_RATE)
  glen = glen.to(device)
  ...
  for epoch in range(NUM_EPOCH):
    ...
    if USE_MEMORY:
      glen.memory.__init_memory__()

    # Train using only training graph
    glen.set_neighbor_finder(train_ngh_finder)
    m_loss = []
    ...
    for k in range(0, num_batch, args.backprop_every):
      ...
      for j in range(args.backprop_every):
        ...
        glen = glen.train()
        pos_prob = glen.compute_edge_probabilities_modified(sources_batch, destinations_batch, timestamps_batch,
                                            edge_idxs_batch, True, NUM_NEIGHBORS)
        neg_prob = glen.compute_edge_probabilities_modified(sources_batch, negatives_batch, timestamps_batch,
    ...
    glen.set_neighbor_finder(full_ngh_finder)

    if USE_MEMORY:
      # Backup memory at the end of training, so later we can restore it and use it for the
      # validation on unseen nodes
      train_memory_backup = glen.memory.backup_memory()

    val_ap, val_auc, val_measures_dict = eval_edge_prediction_modified(model=glen,
    ...
    if USE_MEMORY:
      ...
      glen.memory.restore_memory(train_memory_backup)

    # Validate on unseen nodes
    nn_val_ap, nn_val_auc, nn_val_measures_dict = eval_edge_prediction_modified(model=glen,
                                                                        negative_edge_sampler=val_rand_sampler,
    ...
    if early_stopper.early_stop_check(val_ap):
      ...
      glen.load_state_dict(torch.load(best_model_path))
      logger.info(f'Loaded the best model at epoch {early_stopper.best_epoch} for inference')
      glen.eval()
      break
    else:
      torch.save(glen.state_dict(), get_checkpoint_path(epoch))
  ...
  torch.save(glen.state_dict(), MODEL_SAVE_PATH)
  logger.info('GLEN model saved')
