...
parser = argparse.ArgumentParser('GLEN supervised training')
...
MODEL_SAVE_PATH = f'./saved_models/{args.prefix}-{args.data}-nc.pth'
get_checkpoint_path = lambda \
    epoch: f'./saved_checkpoints/{args.prefix}-{args.data}-{epoch}-nc.pth'

### set up logger
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)
fh = logging.FileHandler('log/{}_node_classifiction.log'.format(args.data))
...
for i in range(args.n_runs):
  ...
  logger.info('Loading saved glen model')
  model_path = f'./saved_models/{args.prefix}-{DATA}.pth'
  #model_path = f'./saved_checkpoints/{args.prefix}-{DATA}-6.pth'
  glen.load_state_dict(torch.load(model_path))
  glen.eval()
  logger.info('glen models loaded')
  ...
  for epoch in range(args.n_epoch):
    ...
    for k in range(num_batch):
      ...
      with torch.no_grad():
        source_embedding, destination_embedding, _ = glen.compute_temporal_embeddings_original(sources_batch,
                                                                                     destinations_batch,
                                                                                     destinations_batch,
                                                                                     timestamps_batch,
                                                                                     edge_idxs_batch,
    ...
    val_auc = eval_node_classification(glen, decoder, val_data, full_data.edge_idxs, BATCH_SIZE,
  ...
  if args.use_validation:
    ...
    test_auc = eval_node_classification(glen, decoder, test_data, full_data.edge_idxs, BATCH_SIZE,
                                        n_neighbors=NUM_NEIGHBORS)
  else:
